...
**Retirar uma emissão já publicada.** Quando uma linha está errada, a ligação da prova é escrita na lista `remover` de `config/curadoria.yml`, com o motivo. A emissão deixa de ser contada e passa a aparecer na quarentena com esse motivo escrito, onde continua visível para quem a queira ver. Isto é a única exceção à regra de que o conjunto de dados só cresce, e é deliberada: essa regra existe para que uma emissão não desapareça quando a fonte que a produzia deixa de a devolver, e uma remoção à mão é o contrário disso, é alguém a afirmar que a linha está errada. O motivo é obrigatório, para que uma remoção nunca seja indistinguível de um descuido.
